...
/*----------------------------------------------------------------------------- */
static int fatfs_find_free_dir_offset(struct fatfs *fs, uint32_t dirCluster, int entryCount, uint32_t *pSector, unsigned char *pOffset)
{
...
int fatfs_add_file_entry(struct fatfs *fs, uint32_t dirCluster, char *filename, char *shortfilename, uint32_t startCluster, uint32_t size, int dir)
{
	...
	while (true)
	{
		...
		if (fatfs_sector_reader(fs, dirCluster, x++, false))
		{
			...
			if (dirtySector)
			{
				if (!fs->disk_io.write_sector(fs->currentsector.address, fs->currentsector.sector, fs->fs_fd))
					return 0;
