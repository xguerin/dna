...
void fatfs_sfn_create_entry(char *shortfilename, uint32_t size, uint32_t startCluster, FAT32_ShortEntry *entry, int dir)
{
	...
	entry->FstClusHI = (uint16_t)((startCluster>>16) & 0xFFFF);
	entry->FstClusLO = (uint16_t)((startCluster>>0) & 0xFFFF);
	entry->FileSize = size;
}
#endif
/*----------------------------------------------------------------------------- */
...
int fatfs_lfn_create_sfn(char *sfn_output, char *filename)
{
	...
	for (i=0;i<len;i++)
	{
		if ( (filename[i]!=' ') && (filename[i]!='.') )
			sfn_output[pos++] = (char)toupper((int)filename[i]);
	...
	/* Add extension part */
	for (i=8;i<11;i++)
		sfn_output[i] = (char)toupper((int)ext[i-8]);
