...
{
 	uint32_t SectortoWrite = 0;
	uint32_t ClustertoWrite = 0;
	uint32_t ClusterChain = 0;
	uint32_t LastClusterChain = FAT32_INVALID_CLUSTER;
	uint32_t i;
	...
	if (fs->fat_type == FAT_TYPE_16 && Startcluster == 0)
	{
		...
		{
			/* Write to disk */
			return fs->disk_io.write_sector(lba, target, fs->fs_fd);
		}
		else
		{
			...
			/* Write to disk */
			return fs->disk_io.write_sector(fs->currentsector.address, fs->currentsector.sector, fs->fs_fd);
		}
	}
	/* FAT16/32 Other */
	else
	{
		...
		{
			/* Calculate write address */
			uint32_t lba = fatfs_lba_of_cluster(fs, ClusterChain) + SectortoWrite;
...
void fatfs_show_details(struct fatfs *fs)
{
	...
	FAT_PRINTF(("0x%x",(unsigned int)fs->rootdir_first_cluster));
	FAT_PRINTF(("\r\nFAT Begin LBA = "));
	FAT_PRINTF(("0x%x",(unsigned int)fs->fat_begin_lba));
	FAT_PRINTF(("\r\nCluster Begin LBA = "));
	FAT_PRINTF(("0x%x",(unsigned int)fs->cluster_begin_lba));
	FAT_PRINTF(("\r\nSectors Per Cluster = "));
	FAT_PRINTF(("%d",(unsigned int)fs->sectors_per_cluster));
		...
		if (fatfs_sector_reader(fs, Cluster, x++, false)) /* If sector read was successfull */
		{
			...
			for (item = 0; item < 16; item++)
			{
				...
				else if (fatfs_entry_lfn_exists(&lfn, directoryEntry) )
				{
					...
					if (fatfs_compare_names((char *)LongFilename, (char *)nametofind))
					{
		...
		if (fatfs_sector_reader(fs, dirls->cluster, dirls->sector, false))
		{
			...
			for (item = dirls->offset; item < 16; item++)
			{
				...
				else if (fatfs_entry_lfn_exists(&lfn, directoryEntry) )
				{
					/* Get text */
					LongFilename = fatfs_lfn_cache_get(&lfn);
					strncpy(entry->filename, (char *)LongFilename, FATFS_MAX_LONG_FILENAME-1);
