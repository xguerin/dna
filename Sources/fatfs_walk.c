    ...
    
{
  ...
  log (INFO_LEVEL, "FATFS walk [start]");
  ...
  watch (status_t)
  {
	...
	{
		for(i = 0; i < base_inode -> nb_sector; i++)
		{
			/* Analyse Sector */
			for (item = 0; item < 16; item++)
			{
				...
				directoryEntry = (fatfs_entry_t)(base_inode -> cluster_chain_directory + i + recordoffset);

				/* Long File Name Text Found */
				if (fatfs_entry_lfn_text(directoryEntry) )
					fatfs_lfn_cache_entry(&lfn, base_inode -> cluster_chain_directory + i + recordoffset);
				...
				else if (fatfs_entry_lfn_exists(&lfn, directoryEntry) )
				{
					...
					{
						vnid = ((uint64_t)base_inode -> cc_dirid << 32) + item + i*16;
						break;
					}
